...
static int make_random(RadixMap * map)
{
  ...
  for (i = 0; i < map->max -1; i++) {
    ...
    check(RadixMap_add(map, key, i) == 0, "Failed to add %u", key);
  }
...
static int check_order(RadixMap * map)
{
  ...
  for (i = 0; map->end > 0 && i < map->end -1; i++) {
    ...
    if (d1.data.key > d2.data.key) {
      debug("FAIL:i=%u, key: %u, value: %u, equals max? %d/n", i,
...
static int test_search(RadixMap * map)
{
  ...
  for (i = map->end / 2; i < map->end; i++){
    ...
    check(found->data.key == d->data.key, "gor the wrong result: %p:%u looking for %u at %u" found,
...
static char *test_operations()
{
  ...
  while (map->end > 0) {
    ...
    mu_assert(RadixMap_delete(map, el) == 0, "didn't size after delete.");

    //test that the end is now the old value,
    //but uint32 max so it traikls off 
